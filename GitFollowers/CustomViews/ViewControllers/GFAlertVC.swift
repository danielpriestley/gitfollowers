...
class GFAlertVC: UIViewController {
    ...
    let containerView = GFAlertContainerView(backgroundColor: .systemBackground, borderColor: UIColor.white.cgColor)
    let titleLabel = GFTitleLabel(textAlignment: .center, fontSize: 20)
    let messageLabel = GFBodyLabel(textAlignment: .center)
    let actionButton = GFButton(backgroundColor: .systemPink, title: "Okay")
    
    // These are optional and will be initialised in the custom init
    var alertTitle: String?
    var message: String?
    var buttonTitle: String?
    let padding: CGFloat = 20
    
    // This is the custom init
